...
@run: from main.py's root -> python3 main.py data/movie_data.csv

@notes: main is located at the bottom of the file, for understanding I'd reccomend reading that first, then
	move to the top and follow the methods down through the execution path (top_genres, genre_properties, zipfs)
"""
...
# @param: (data) movie_data.csv as a pandas dataframe object
#
# @note: This method is pretty long, for future work breaking this into separate functions would be helpful
...
# @function: zipfs
# @purpose: Investigate all of the summaries and see if the corpus exhibits properties of zipfs law
# @param: (data) movie_data.csv as a pandas dataframe object
def zipfs(data):
	...
	# for future work overlaying a red reference line for a true zipfs distribution could be helpful

# @function: main
# @purpose: driver function that reads in the dataset and calls all other functions
def main():
	# check to make sure our dataset was passed in, graceful exit (code 1) if it wasn't
	if len(sys.argv) is not 2:
		print("Error. Please provide this script with the movie dataset as an argument like:")
		print("python3 main.py data/movie_data.csv")
		sys.exit(1)
